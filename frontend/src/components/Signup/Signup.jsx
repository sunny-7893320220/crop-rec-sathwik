...
import { User, Phone, Mail, Lock } from 'lucide-react'; // Added new icons
...
const Signup = () => {
    ...
    if (!validateform()) {
      ...
      try {
        const response = await axios.post("http://localhost:8000/api/v1/auth/signup", {
          farmersName, farmersPhone, farmersEmail, password
        });
        ...
        if (error.response?.headers['content-type']?.includes('application/json')) {
          ...
          handleError(error.response ? "A conflict occurred: " + error.response.statusText : "An unexpected error occurred.");
        }
      }
    }
  };
  ...
  const validateform = () => {
    ...
    if (!signUpValues.farmersPhone) {
      newErrors.farmersPhone = 'PhoneNumber is required';
    } else if (!/^\d{10}$/.test(signUpValues.farmersPhone)) { // Example phone validation
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen relative mx-5 my-5 rounded-lg shadow-2xl" id="signup">
      <div className="bg-white p-6 md:p-10 flex flex-col justify-center w-full lg:w-1/2 rounded-lg">
        <h1 className="text-3xl md:text-4xl lg:text-6xl font-bold mb-6 p-5 text-center signika-font"
      ...
      <div className="hidden lg:block w-full lg:w-1/2 bg-cover bg-center rounded-lg"
